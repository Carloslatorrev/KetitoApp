...
public interface IDestinatarioApi {
    ...
    @GET(TagApi.API_URL_BASE + TagApi.API_URL_PATH_DESTINATARIO_FIND)
    Call<ResponseBody> findDestinatario(@Header(TagApi.API_HEADER_CONTENT_TYPE_NAME) String contentType,
                                        @Header(TagApi.API_HEADER_AUTHORIZATION_NAME) String authorization,
                                        @Path("movil") String movil);
